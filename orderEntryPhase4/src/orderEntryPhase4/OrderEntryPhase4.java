// Enrique J Simon II CIS406 Order Entry Phase 4
...
public class OrderEntryPhase4 {
	...
	public static void main(String[] args) {
		...
		List<String> labelList_cust = new ArrayList<>();
		List<String> labelList_data = new ArrayList<>();
		...
		String custAddy = scanner.nextLine();
		...
		String custZip = scanner.nextLine();
		System.out.print("\n");
		
		// Append List: Customer Info
		custInfo.add(custName);
		custInfo.add(custAddy);
		custInfo.add(custCity);
		...
		while (exitEntry.equals("y")) {
			...
			float disAmt = ((grsAmt + taxAmt) * itemDiscount) / 100;
			float netAmt = (grsAmt + taxAmt) - disAmt;
		...
		// ---------- Labels & Print Data ----------
		// Create: Customer Info Headers
		String custName_Lab = "    Customer Name    ";
		labelList_cust.add(custName_Lab);
		String custAddy_Lab = "    Customer Street    ";
		labelList_cust.add(custAddy_Lab);
		String custCity_Lab = "    Customer City    ";
		labelList_cust.add(custCity_Lab);
		String custState_Lab = "    Customer State    ";
		labelList_cust.add(custState_Lab);
		String custZip_Lab = "    Customer Zip    ";
		labelList_cust.add(custZip_Lab);
		
		
		// Create: Data Headers
		String itemNum_Lab = "Item Number";
		labelList_data.add(itemNum_Lab);
		String itemDes_Lab = "Item Description";
		labelList_data.add(itemDes_Lab);
		String itemPri_Lab = "Item Price";
		labelList_data.add(itemPri_Lab);
		String itemQuan_Lab = "Quantity";
		labelList_data.add(itemQuan_Lab);
		String itemTaxP_Lab = "Tax Percentage";
		labelList_data.add(itemTaxP_Lab);
		String itemTaxA_Lab = "Tax Amount";
		labelList_data.add(itemTaxA_Lab);
		String itemGrsA_Lab= "Gross Amount";
		labelList_data.add(itemGrsA_Lab);
		String itemDisP_Lab = "Discount Percentage";
		labelList_data.add(itemDisP_Lab);
		String itemDisA_Lab = "Discount Amount";
		labelList_data.add(itemDisA_Lab);
		String itemNetA_Lab = "Net Amount";
		labelList_data.add(itemNetA_Lab);
		...
		// Print: Invoice Header (Customer Info)
		System.out.println(custName_Lab + " -------- " + 
				   		   custAddy_Lab + " -------- " +
				   		   custCity_Lab + " -------- " +
				   		   custState_Lab + " -------- " +
				   		   custZip_Lab);
		...
		for (int i = 0; i < labelList_cust.size() && i < custInfo.size(); i++) {
			...
			String dataSelect = String.valueOf(custInfo.get(i));
		...
		for (List masterEntry : masterJournal) {
			...
			for (int i = 0; i < labelList_data.size() && i < dataList.size(); i++) {
				// Get Data
				String headerSelect = labelList_data.get(i);
		...
		System.out.println(bannerChar.repeat(bannerLen));
		for (int i = 0; i < labelList_data.size() && i < totalsEntry.size(); i++) {
			// Get Data
			String headerSelect = labelList_data.get(i);
